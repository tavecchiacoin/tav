...
CBlock BuildBlock(const CChainParams& params, const benchmark::ScriptRecipe& rec, const uint256& seed)
{
    ...
    const auto& taproot_deployment{params.GetConsensus().vDeployments[Consensus::DEPLOYMENT_TAPROOT]};
    assert(params.GetConsensus().SegwitHeight <= GENERATED_BLOCK_HEIGHT);
    assert(taproot_deployment.nStartTime == Consensus::BIP9Deployment::ALWAYS_ACTIVE);
    assert(taproot_deployment.min_activation_height <= GENERATED_BLOCK_HEIGHT);
