...
//! https://github.com/llvm/llvm-project/issues/12658 which skips calling destructors
//! and can lead to memory leaks. Also, if both functions throw, it lets the
//! cleanup exception take precedence instead of terminating.
template <typename Fn, typename After>
decltype(auto) CallThen(Fn&& fn, After&& after)
{
    bool done{false};
    ...
    try {
        ...
        if (!done) std::forward<After>(after)();
        throw;
    }
}
