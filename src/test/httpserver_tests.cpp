...
{
    {
        ...
        BOOST_CHECK_EQUAL(req.m_headers.FindFirst("Host").value(), "127.0.0.1");
        BOOST_CHECK_EQUAL(req.m_headers.FindFirst("Connection").value(), "close");
        BOOST_CHECK_EQUAL(req.m_headers.FindFirst("Content-Type").value(), "application/json");
        BOOST_CHECK_EQUAL(req.m_headers.FindFirst("Authorization").value(), "Basic X19jb29raWVfXzo5OGQ5ODQ3MWNmNjg0NzAzYTkzN2EzNzk0ZDFlODQ1NjZmYTRkZjJiMzFkYjhhODI4ZGY4MjVjOTg5ZGI4OTVl");
        BOOST_CHECK_EQUAL(req.m_headers.FindFirst("Content-Length").value(), "46");
    ...
    }
    {
        ...
        const std::string malformed_request_line = "GET / HTTP/2.0\r\nHost: 127.0.0.1\r\n\r\n";
        HTTPRequest req;
        std::vector<std::byte> buffer{StringToBuffer(malformed_request_line)};
        LineReader reader(buffer, MAX_HEADERS_SIZE);
        BOOST_CHECK_EXCEPTION(req.LoadControlData(reader), std::runtime_error, HasReason{"HTTP bad version"});
