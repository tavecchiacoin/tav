...
class WalletLogSource : public util::log::Source
{
    std::string m_display_name;

public:
    WalletLogSource(std::string display_name) : m_display_name{std::move(display_name)} {}

    template <typename... Args>
    std::string Format(util::ConstevalFormatString<sizeof...(Args)> fmt, const Args&... args) const
    {
        return "[" + m_display_name + "] " + Source::Format(fmt, args...);
    }
};
} // namespace wallet
