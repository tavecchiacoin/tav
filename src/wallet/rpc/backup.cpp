...
struct DescriptorImportData {
    std::string descriptor;
    bool active;
    std::string label;
    std::vector<std::unique_ptr<Descriptor>> parsed_descs;
    ...
    DescriptorImportData() = default;
    ~DescriptorImportData() = default;
    DescriptorImportData(const DescriptorImportData&) = delete;
    DescriptorImportData& operator=(const DescriptorImportData&) = delete;
    DescriptorImportData(DescriptorImportData&&) = default;
    DescriptorImportData& operator=(DescriptorImportData&&) = default;
};

static DescriptorImportData ValidateDescriptorImport(CWallet& wallet, const UniValue& data) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{

    DescriptorImportData result;
    try {
        if (!data.exists("desc")) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Descriptor not found.");
        }
        result.descriptor = data["desc"].get_str();
        result.active = data.exists("active") ? data["active"].get_bool() : false;
        result.label = LabelFromValue(data["label"]);

        // Parse descriptor string
        std::string error;
        result.parsed_descs = Parse(result.descriptor, result.keys, error, /* require_checksum = */ true);
        if (result.parsed_descs.empty()) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, error);
        }
        if (data.exists("internal")) {
            if (result.parsed_descs.size() > 1) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Cannot have multipath descriptor while also specifying \'internal\'");
            }
            result.internal = data["internal"].get_bool();
        }
        // Range check
        ...
        if (!result.parsed_descs.at(0)->IsRange() && data.exists("range")) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Range should not be specified for an un-ranged descriptor");
        } else if (result.parsed_descs.at(0)->IsRange()) {
            if (data.exists("range")) {
                auto range = ParseDescriptorRange(data["range"]);
                result.range_start = range.first;
                result.range_end = range.second + 1; // Specified range end is inclusive, but we need range end as exclusive
            } else {
                result.warnings.push_back("Range not given, using default keypool range");
                result.range_start = 0;
                result.range_end = wallet.m_keypool_size;
            }
            result.next_index = result.range_start;
            is_ranged = true;

            if (data.exists("next_index")) {
                result.next_index = data["next_index"].getInt<int64_t>();
                // bound checks
                if (result.next_index < result.range_start || result.next_index >= result.range_end) {
        ...
        if (result.active && !result.parsed_descs.at(0)->IsRange()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Active descriptors must be ranged");
        }
        // Multipath descriptors should not have a label
        if (result.parsed_descs.size() > 1 && data.exists("label")) {
        ...
        bool desc_internal = result.internal.has_value() && result.internal.value();
        // Internal addresses should not have a label either
        if (desc_internal && data.exists("label")) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Internal addresses should not have a label");
        }
        // Combo descriptor check
        if (result.active && !result.parsed_descs.at(0)->IsSingleType()) {
            throw JSONRPCError(RPC_WALLET_ERROR, "Combo descriptors cannot be set to active");
        }
        // If the wallet disabled private keys, abort if private keys exist
        if (wallet.IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS) && !result.keys.keys.empty()) {
            throw JSONRPCError(RPC_WALLET_ERROR, "Cannot import private keys to a wallet with private keys disabled");
        }
        for (size_t j = 0; j < result.parsed_descs.size(); ++j) {
            const auto& parsed_desc = result.parsed_descs[j];
            // Need to ExpandPrivate to check if private keys are available for all pubkeys
            FlatSigningProvider expand_keys;
            std::vector<CScript> scripts;
            if (!parsed_desc->Expand(0, result.keys, scripts, expand_keys)) {
                throw JSONRPCError(RPC_WALLET_ERROR, "Cannot expand descriptor. Probably because of hardened derivations without private keys provided");
            }
            parsed_desc->ExpandPrivate(0, result.keys, expand_keys);
            ...
            if (!wallet.IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
                if (result.keys.keys.empty()) {
                    throw JSONRPCError(RPC_WALLET_ERROR, "Cannot import descriptor without private keys to a wallet with private keys enabled");
                }
                if (!have_all_privkeys) {
                    result.warnings.push_back("Not all private keys provided. Some wallet functionality may return unexpected errors");
                }
            }
        }
        result.success = true;
...
static UniValue ProcessDescriptorImport(CWallet& wallet,  DescriptorImportData& import_data, const int64_t timestamp) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    UniValue warnings = import_data.warnings;
    UniValue result(UniValue::VOBJ);

    try {
        for (size_t j = 0; j < import_data.parsed_descs.size(); ++j) {
            auto parsed_desc = std::move(import_data.parsed_descs[j]);
            bool desc_internal = import_data.internal.has_value() && import_data.internal.value();
            if (import_data.parsed_descs.size() == 2) {
                desc_internal = j == 1;
            } else if (import_data.parsed_descs.size() > 2) {
                CHECK_NONFATAL(!desc_internal);
            }
            WalletDescriptor w_desc(std::move(parsed_desc), timestamp, import_data.range_start, import_data.range_end, import_data.next_index);

            // Add descriptor to the wallet
            auto spk_manager_res = wallet.AddWalletDescriptor(w_desc, import_data.keys, import_data.label, desc_internal);

            if (!spk_manager_res) {
                throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Could not add descriptor '%s': %s", import_data.descriptor, util::ErrorString(spk_manager_res).original));
            }

            auto& spk_manager = spk_manager_res.value().get();

            // Set descriptor as active if necessary
            if (import_data.active) {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& main_request) -> UniValue
{
    ...
    std::vector<DescriptorImportData> validated_imports;
    validated_imports.reserve(requests.size());
    {
        LOCK(pwallet->cs_wallet);
        EnsureWalletIsUnlocked(*pwallet);

        UniValue validation_response(UniValue::VARR);
        bool all_valid = true;
        ...
        for (const UniValue& request : requests.getValues()) {
            ...
            if (!validated.success) {
                result.pushKV("error", validated.error);
                all_valid=false;
            }
            if (validated.warnings.size() > 0) {
                result.pushKV("warnings", validated.warnings);
            }
            validation_response.push_back(result);
            validated_imports.push_back(std::move(validated));
        }
        if(!all_valid)
        {
        ...
        for (size_t i = 0; i < validated_imports.size(); ++i) {
            // This throws an error if "timestamp" doesn't exist
            const UniValue& request = requests.getValues().at(i);
            const int64_t timestamp = std::max(GetImportTimestamp(request, now), minimum_timestamp);
            const UniValue result = ProcessDescriptorImport(*pwallet, validated_imports[i], timestamp);
